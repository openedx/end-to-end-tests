...
login is limited to 30 per account per 5 minutes by default. The fixtures encode
the survival strategy, so the rules for a spec are short:

- Depend on `studioAuthorSession` for browser work. There is no course-deletion
  API, so pick the existing course that fits and never create one you do not
  have to:

  | Fixture           | Scope  | Use for                                                                   |
  | ----------------- | ------ | ------------------------------------------------------------------------- |
  | `authoredCourse`  | worker | Settings specs that move the schedule and enrollment window around        |
  | `contentCourse`   | worker | Authoring round trips; build your own `ownSection`, never touch another's |
  | `futureCourse`    | worker | Not-yet-started course, with `futureCourseLearner`                        |
  | `authoringCourse` | test   | Fresh, empty course for specs that build the outline through the UI       |
  | `lifecycleCourse` | test   | Only when course creation itself is the subject                           |

- **Same user in browser and API in one test: `page.request`, never `request`.**
  Pattern: `tests/studio/home/course-lifecycle.spec.ts`.
- **Different user: different context.** Learners via `roundTripLearner` (or
  `roundTripLearners` for two), which hold their own browser and request
  contexts. The admin via `adminPage` or `adminApi`, which hold the cross-worker
  admin lock. Never sign a second user in on the author's `page`, and never sign
  in as a shared account outside `withAdminSession`.
- LMS session-auth views (cohorts, instructor dashboard) reject JWT-only
  ...
  context after the browser work, and dispose it in `finally`. Pattern:
  `tests/studio/outline/sidebar.spec.ts`, client in `src/api/cohorts.ts`.
- Legacy CMS writes (`/course/`, `/xblock/`) go through `studioWrite`, so a dead
  session is a typed `StudioSessionExpiredError` rather than a followed redirect.
  A per-test write on `request` with no browser session in the test calls
  `establishAuthorWriteSession` first.
...
Platform facts, failure signatures, and the approaches already tried and
reverted: [`studio-auth.md`](skills/openedx-e2e/references/studio-auth.md).
