...
/**
 * Marks a subsection as available as a prerequisite (or not). Unlike every other
 * field this is a **top-level** key of the write body, not `metadata` — the
 * gating fields are stored by the milestones app, not on the block.
 */
export async function setAvailableAsPrerequisite(
  request: APIRequestContext,
  config: AppConfig,
  sequentialUsageKey: string,
  isPrereq: boolean,
): Promise<void> {
  await studioWrite(
    request,
    config,
    'PATCH',
    `${XBLOCK_PATH}${sequentialUsageKey}`,
    `Marking ${sequentialUsageKey} as ${isPrereq ? 'a' : 'no'} prerequisite`,
    { isPrereq },
  );
}

export interface PrerequisiteRequirement {
  /** The subsection that must be satisfied first, or `''` to remove the gate. */
  readonly prerequisiteUsageKey: string;
  /** Minimum score in the prerequisite, 0–100. */
  readonly minScore: number;
  /** Minimum completion of the prerequisite, 0–100. */
  readonly minCompletion: number;
}

/**
 * Gates a subsection behind another. **Both thresholds must be met** when both
 * are non-zero (measured: a 100 % score never opened a gate that also asked for
 * 100 % completion of a subsection with a video in it), so a spec about one
 * threshold sets the other to 0. Top-level keys like {@link setAvailableAsPrerequisite}.
 */
export async function requirePrerequisite(
  ...
  await studioWrite(
    ...
    'PATCH',
    `${XBLOCK_PATH}${sequentialUsageKey}`,
    `Gating ${sequentialUsageKey} behind ${requirement.prerequisiteUsageKey || 'nothing'}`,
    {
      prereqUsageKey: requirement.prerequisiteUsageKey,
      prereqMinScore: requirement.minScore,
      prereqMinCompletion: requirement.minCompletion,
    },
  );
}

/** Duplicates a block next to itself under `parentLocator`; returns the copy's key. */
export async function duplicateXBlock(
  ...
  const body = await studioWrite<CreateXBlockResponse>(
    ...
    'POST',
    XBLOCK_PATH,
    `Duplicating ${usageKey}`,
    { duplicate_source_locator: usageKey, parent_locator: parentLocator },
  );
  if (typeof body?.locator !== 'string') {
    throw new ApiError(`Studio duplicated ${usageKey} but returned no locator.`, {
      status: 200,
      url: `${studioOrigin(config)}${XBLOCK_PATH}`,
      body: JSON.stringify(body),
    });
...
/** Deletes a block and everything below it. */
export async function deleteXBlock(
  request: APIRequestContext,
  config: AppConfig,
  usageKey: string,
): Promise<void> {
  await studioWrite(request, config, 'DELETE', `${XBLOCK_PATH}${usageKey}`, `Deleting ${usageKey}`);
}

/**
 * Sets the order of a block's children — the request both keyboard drag-and-drop
 * and the Move up / Move down menu items send. A key that currently belongs to
 * another parent is **moved** here, so callers pass exactly the parent's own
 * children.
 */
export async function reorderChildren(
  ...
  await studioWrite(
    ...
    config,
    'PUT',
    `${XBLOCK_PATH}${parentUsageKey}`,
    `Reordering the children of ${parentUsageKey}`,
    { children: orderedChildren },
  );
}

/** Moves a block under another parent at `targetIndex`. */
export async function moveXBlock(
  ...
  await studioWrite(request, config, 'PATCH', XBLOCK_PATH, `Moving ${usageKey}`, {
    move_source_locator: usageKey,
