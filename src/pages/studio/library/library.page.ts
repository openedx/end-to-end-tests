...
export class LibraryPage {
  ...
  async addComponent(type: Exclude<AddableComponentType, 'advanced'>): Promise<void> {
    await this.page
      ...
      .click();
    await this.page.locator(this.s.editorDialog).waitFor({ timeout: TIMEOUTS.navigation });
  }
  ...
  async closeEditor(): Promise<void> {
    ...
    await this.page.locator(this.s.editorDialog).waitFor({ state: 'hidden' });
  }

  /**
   * "Advanced / Other" then the advanced type at `index` in the list it shows
   * (the library's `block_types` minus the basic ones, in API order — the
   * caller computes the index from `fetchLibraryBlockTypes`), waiting for the
   * `POST <lib>/blocks/`.
   */
  async addAdvancedComponent(index: number): Promise<Response> {
    await this.page
      .locator(this.s.addContentButtonAfterRule)
      .nth(this.s.addComponentIndex.advanced)
      .click();
    return waitForWrite(
      this.page,
      { method: 'POST', urlIncludes: '/blocks/', timeout: TIMEOUTS.contentWrite },
      () => this.page.locator(this.s.advancedTypeButton).nth(index).click(),
    );
  }
  ...
  async pasteFromClipboard(): Promise<Response> {
    return waitForWrite(
      this.page,
      { method: 'POST', urlIncludes: '/paste_clipboard/', timeout: TIMEOUTS.contentWrite },
      () => this.page.locator(this.s.addContentButtonAfterRule).last().click(),
    );
  }
  ...
  async search(term: string): Promise<void> {
    await Promise.all([this.waitForSearch(), this.searchInput.fill(term)]);
  ...
  /** Waits for one search (Meilisearch multi-search) response — what every filter, sort and keyword change fires. */
  waitForSearch(): Promise<Response> {
    return this.page.waitForResponse(
      (r) =>
        r.request().method() === 'POST' && /\/(multi-search|indexes\/[^/]+\/search)/.test(r.url()),
  ...
  async sortBy(option: keyof typeof LIBRARY_SELECTORS.sortOption): Promise<void> {
    ...
    await Promise.all([
      this.waitForSearch(),
      this.page.locator(this.s.sortMenuItem).nth(this.s.sortOption[option]).click(),
    ]);
  }
  ...
  async toggleTypeFilter(blockType: string): Promise<void> {
    ...
    await Promise.all([this.waitForSearch(), box.click()]);
  }
  ...
  async toggleTagFilter(index: number): Promise<void> {
    ...
    await Promise.all([this.waitForSearch(), item.click()]);
  }
   ...
   */
  async removeCardFromParent(title: string): Promise<Response> {
    await this.openCardMenu(title);
    await this.page.locator(this.s.menuRemoveItem).first().click();
    return this.confirmDialog('DELETE', '/children/');
  }
